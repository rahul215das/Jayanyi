...
export default function App() {
  ...
  // ADMIN PANEL — controlled inputs for reliable typing + submission
  const Admin = () => {
    ...
    // (Video + About still use refs; Event form is controlled.)

    const handleAddEvent = () => {
      const title = evTitle.trim();
      const date = evDate;
      if (!title || !date) return;
      const updated = { ...data, events: [...data.events, { id: Date.now(), title, date, venue: evVenue || "", description: evDescription || "" }] };
      save(updated);
      setEvTitle("");
      setEvDate("");
      setEvVenue("");
    ...
    return (
      <div style={styles.section}>
        ...
        <div style={{ borderBottom: `2px solid #e8d9b0`, marginBottom: "1.5rem", display: "flex", gap: "0.5rem", overflowX: "auto" }}>
          {["events", "members", "about", "gallery", "videos"].map(t => (
          ...
          <div>
            <div style={styles.card}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.75rem" }}>
                <div><label style={styles.label}>Event Title *</label><input value={evTitle} onChange={e => setEvTitle(e.target.value)} style={styles.input} placeholder="Event name" /></div>
                <div><label style={styles.label}>Date *</label><input value={evDate} onChange={e => setEvDate(e.target.value)} type="date" style={styles.input} /></div>
                <div><label style={styles.label}>Venue</label><input value={evVenue} onChange={e => setEvVenue(e.target.value)} style={styles.input} placeholder="Location/venue" /></div>
                <div><label style={styles.label}>Description</label><input value={evDescription} onChange={e => setEvDescription(e.target.value)} style={styles.input} placeholder="Brief description" /></div>
              </div>
        ...
        {adminTab === "members" && (
          <div>
            <div style={styles.card}>
              <h3 style={{ marginTop: 0, color: colors.red }}>Add Members</h3>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.75rem" }}>
                <div>
                  <label style={styles.label}>Member Name</label>
                  <input value={memberName} onChange={e => setMemberName(e.target.value)} style={{ ...styles.input, cursor: 'text' }} placeholder="Full name" />
                </div>
                <div>
                  <label style={styles.label}>Role / Designation</label>
                  <input value={memberRole} onChange={e => setMemberRole(e.target.value)} style={styles.input} placeholder="e.g. President, Vocalist" />
                </div>
              </div>
              <button
                style={{ ...styles.btn("primary"), marginTop: "1rem" }}
                onClick={() => {
                  if (!memberName.trim()) return;
                  const updated = {
                    ...data,
                    members: [...(data.members || []), { id: Date.now(), name: memberName.trim(), role: memberRole.trim() }]
                  };
                  save(updated);
                  setMemberName("");
                  setMemberRole("");
            ...
            <h3 style={{ color: colors.red }}>Members ({(data.members || []).length})</h3>
            {(!data.members || data.members.length === 0) && <p style={{ color: colors.muted }}>No members added yet.</p>}

            {(data.members || []).length > 0 && (
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(240px,1fr))", gap: "0.75rem" }}>
                {data.members.map(m => (
                  <div key={m.id} style={{ ...styles.card, padding: "1rem" }}>
                    <div style={{ fontWeight: "bold", color: colors.red, marginBottom: "0.35rem" }}>{m.name}</div>
                    {m.role && <div style={{ color: colors.muted, fontSize: "0.95rem", marginBottom: "0.75rem" }}>{m.role}</div>}
                    <button
                      style={{ ...styles.btn("danger"), padding: "0.35rem 0.9rem", fontSize: "0.85rem" }}
                      onClick={() => {
                        const updated = { ...data, members: (data.members || []).filter(x => x.id !== m.id) };
                        save(updated);
    ...
    <div style={styles.app}>
      ...
      <footer style={{ background: colors.dark, color: "#bfa060", textAlign: "center", padding: "2rem 1rem", fontSize: "0.85rem", letterSpacing: "0.05em" }}>
        ...
        <div style={{ marginTop: "1.25rem", paddingTop: "1rem", borderTop: `1px solid rgba(255,255,255,0.12)` }}>
          <div style={{ color: colors.gold, fontSize: "1.05rem", marginBottom: "0.65rem" }}>Our Members</div>
          {(data.members || []).length === 0 ? (
            <div style={{ color: "#7a6040", marginBottom: "0.75rem" }}>Members will be announced soon.</div>
          ) : (
            <div style={{ display: "flex", flexWrap: "wrap", gap: "0.6rem", justifyContent: "center" }}>
              {data.members.map(m => (
                <div key={m.id} style={{ background: "rgba(255,255,255,0.06)", border: `1px solid rgba(255,255,255,0.10)`, borderRadius: 999, padding: "0.35rem 0.7rem", color: "#f7e7b8" }}>
                  <span style={{ fontWeight: "bold", color: colors.gold }}>{m.name}</span>
                  {m.role ? <span style={{ color: "#bfa060" }}> · {m.role}</span> : null}
                </div>
              ))}
            </div>
          )}
